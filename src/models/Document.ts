...
// Methods
documentSchema.methods.updateStatus = function(status: string) {
  this.status = status;
  return this.save();
};

documentSchema.methods.updateExtractedData = function(data: any) {
...
// Statics
documentSchema.statics.findByTaxYear = function(year: number) {
  return this.find({ taxYear: year });
};

documentSchema.statics.findByDateRange = function(startDate: Date, endDate: Date) {
  return this.find({
    uploadedAt: {
      $gte: startDate,
      $lte: endDate
...
documentSchema.statics.searchDocuments = function(query: string) {
  return this.find({
    $text: { $search: query }
  });
};
