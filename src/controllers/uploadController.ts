...
export class UploadController {
  ...
  public getDocuments = async (req: Request, res: Response) => {
    try {
      ...
      let documents;
      if (query) {
        documents = await mongoService.searchDocuments(query as string);
      } else if (year) {
        documents = await mongoService.getDocumentsByYear(Number(year));
      } else if (startDate && endDate) {
        documents = await mongoService.getDocumentsByDateRange(startDate as string, endDate as string);
      } else {
  ...
  public getDocumentStatus = async (req: Request, res: Response) => {
    try {
  ...
  public getExtractedData = async (req: Request, res: Response) => {
    try {
      const { documentId } = req.params;
