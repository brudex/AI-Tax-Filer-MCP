...
interface ProcessDocumentArgs {
  ...
  taxpayerName?: string;
  taxYear?: number;
  extractionMode?: 'standard' | 'detailed' | 'validation';
}
...
interface GetAIStatusArgs {
  testConnection?: boolean;
}

interface ValidateDocumentArgs {
  documentPath: string;
}
...
function createMockUploadedFile(filePath: string): MockUploadedFile {
...
function isProcessDocumentArgs(args: any): args is ProcessDocumentArgs {
  return args && typeof args.documentPath === 'string';
}

function isGenerateReportArgs(args: any): args is GenerateReportArgs {
  return args && args.extractedData && typeof args.extractedData === 'object';
}

function isGetAIStatusArgs(args: any): args is GetAIStatusArgs {
  return args !== undefined;
}

function isValidateDocumentArgs(args: any): args is ValidateDocumentArgs {
  return args && typeof args.documentPath === 'string';
}
...
- **Memory Usage:** ${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)}MB`
          ...
          case 'validate_document': {
            ...
            // Test parsing
            let parseTest = { success: false, textLength: 0, error: null as string | null };
            try {
              const mockFile = createMockUploadedFile(documentPath);
