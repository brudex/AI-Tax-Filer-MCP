...
export class ReportGenerator {
  ...
  async generateReport(data: ExtractedTaxData, documentId?: string) {
    return this.generateTaxReport(data, documentId || `DOC-${Date.now()}`);
  }

  async generateTaxReport(data: ExtractedTaxData, documentId: string) {
    try {
      ...
      const formatField = (label: string, value: any, isDefault: boolean) => {
      ...
      doc.fontSize(10)
         .text(
           'This is an automatically generated report. For official purposes, please verify with GRA.',
           doc.page.margins.left,
           doc.page.height - bottomMargin - 20,
           { align: 'center' }
         );
